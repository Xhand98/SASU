...
class Database:
    ...
    def __init__(
        self,
        sqlite3_connection: sqlite3.Connection | str,
        auto_commit: bool = True,
        adapter: DBAdapter = SQLiteAdapter(),
    ):
        self.__database: sqlite3.Connection = (
            sqlite3.connect(sqlite3_connection)
            if isinstance(sqlite3_connection, str)
            else sqlite3_connection
        )
    ...
    def simple_select_data(
        self,
        table: str,
        columns: str = "*",
        conditions: str = "",
        one_fetch: bool = False,
    ) -> list[tuple] | tuple:
        """
    ...
    def simple_insert_data(
        self, table: str, data: tuple | list, especific_columns: str | None = None
    ):
        """
        ...
        if bool(especific_columns):
            self.__cursor.execute(
                f"INSERT INTO {table} ({especific_columns}) VALUES ({', '.join(['?' for _ in data])})",
                data,
            )
            return
    ...
    def simple_add_column(
        self, table_name: str, column_name: str, datatype: type | str
    ):
        """
    ...
    def custom_execute(
        self, query: str, *args: tuple | list | None
...
def generate_id(
    length: int = 18, contains_letters: bool = False, only_letters: bool = False
):
    """
